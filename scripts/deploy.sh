...
do
  ...
  if aws lambda update-function-code --function-name tech-challenge-$folder_name --zip-file fileb://$folder_name.zip --no-cli-pager; then
    echo "Lambda update completed for $folder"
  elif aws lambda create-function --function-name tech-challenge-$folder_name --runtime nodejs18.x --handler index.handler --role arn:aws:iam::$AWS_ACCOUNT_ID:role/LabRole --zip-file fileb://$folder_name.zip --environment "Variables={USER=$DB_USER,HOST='$DB_AWS_HOST',DATABASE='$DB_DATABASE',PASSWORD='$DB_PASSWORD',PORT='$DB_PORT'}" --no-cli-pager; then
